let itemCnt = 0
// @ts-ignore
const webSocket: WebSocket = new WebSocket(`ws://localhost:${window.WSPORT}`);
webSocket.onmessage = (event) => {
    const theJson: { data: {}, metadata: {} } = JSON.parse(event.data);
 ...
 */
function addSubmissionItem(theJson: { data: {}, metadata: {} }) {
    itemCnt += 1
 ...
 */
function AccordionItem(theJson: string): string {
    return `
